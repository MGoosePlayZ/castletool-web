# Terms of Service

**Last Updated: June 15, 2026**

## 1. Acceptance

By accessing or using CastleTool API, you agree to these Terms of Service.

If you do not agree to these Terms, you must not use the service.

## 2. User Responsibility

Users are solely responsible for:

* protecting their Personal Access Tokens;
* ensuring they have permission to access and modify any decks involved;
* complying with applicable laws and the terms governing Castle and related services;
* maintaining backups of important data.

## 3. Security Notice

Personal Access Tokens grant significant access to decks associated with those credentials.

Users should:

* keep their tokens confidential;
* avoid sharing tokens with third parties;
* revoke tokens immediately if compromise is suspected.

CastleTool API cannot guarantee protection against every possible attack or security vulnerability.

## 4. Intellectual Property

Unless otherwise stated, the CastleTool website, frontend, backend software, design, documentation, and associated materials are proprietary.

No permission is granted to copy, reproduce, redistribute, reverse engineer, modify, or create derivative works from the website or software without prior written permission.

All rights are reserved.

## 5. Prohibited Uses

Users may not:

* attempt unauthorized access to the service;
* interfere with service operation;
* abuse, overload, or disrupt the infrastructure;
* use the service for unlawful purposes;
* impersonate other individuals;
* circumvent security mechanisms.

## 6. No Warranty

CastleTool API is provided "AS IS" and "AS AVAILABLE".

No warranties, express or implied, are provided, including warranties of:

* merchantability;
* fitness for a particular purpose;
* reliability;
* availability;
* uninterrupted operation;
* freedom from defects or vulnerabilities.

## 7. Limitation of Liability

To the fullest extent permitted by law, the operators of CastleTool API shall not be liable for:

* loss of data;
* corruption of decks;
* unauthorized access to credentials;
* service interruptions;
* indirect, incidental, consequential, or special damages arising from use of the service.

Users assume all risk associated with use of the service.

## 8. Suspension and Termination

Access to the service may be suspended or terminated at any time, with or without notice, for abuse or violation of these Terms.

## 9. Changes

These Terms may be modified periodically.

Continued use of the service after changes become effective constitutes acceptance of the revised Terms.

## 10. Governing Law

These Terms shall be governed by the laws applicable in the jurisdiction in which the service operator resides.

## 11. Entire Agreement

These Terms constitute the entire agreement between users and the operators of CastleTool API regarding use of the service.

...
* Uploaded MIDI files;
* Request parameters necessary for processing.

This information is processed solely for the purpose of fulfilling the user's request.
...
Temporary files created during processing are automatically removed immediately after the request completes, including when an operation fails.

## 4. Personal Access Tokens

Personal Access Tokens are transmitted to the server only to authenticate operations requested by the user.
...
Images, MIDI files, and other uploaded content are processed only for the purpose of modifying or creating Castle decks.
...
Reasonable measures are taken to protect the service and minimize the amount of information retained.

However, no method of electronic transmission or storage can be guaranteed to be completely secure, and users acknowledge that use of the service is at their own risk.
